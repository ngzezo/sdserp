# SDS ERP Docker Image for Ubuntu 24.04 ARM64
# Based on frappe_docker custom image structure

ARG PYTHON_VERSION=3.11.6
ARG DEBIAN_BASE=bookworm
FROM python:${PYTHON_VERSION}-slim-${DEBIAN_BASE} AS base

LABEL maintainer="SDS ERP Team"
LABEL description="SDS ERP - Open Source ERP System"

# Copy nginx configuration files
COPY docker/nginx-template.conf /templates/nginx/frappe.conf.template
...
# Create frappe user and install base dependencies
RUN useradd -ms /bin/bash frappe \
    ...
    # Install wkhtmltopdf with patched qt (ARM64 compatible)
...
# Build SDS ERP (based on ERPNext)
ARG FRAPPE_BRANCH=version-15
ARG FRAPPE_PATH=https://github.com/frappe/frappe
ARG ERPNEXT_BRANCH=version-15
ARG ERPNEXT_REPO=https://github.com/frappe/erpnext

# Initialize bench with frappe and install erpnext
RUN bench init \
...
# Copy our white-labeled branding changes
